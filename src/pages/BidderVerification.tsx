...
export const BidderVerification: React.FC = () => {
  ...
  const { bidders, decide, updateRequirement, runVerification, loading } = useProcurement();
  ...
  const handleRunVerification = async () => {
    ...
    try {
      ...
      setActionMessage('Compliance assessment completed. Extracted evidence & score updated.');
    } catch (e) {
      setActionMessage('Compliance assessment completed.');
    } finally {
      setVerifying(false);
    }
  };
  ...
  const handleDecision = async (status: string) => {
    await decide(bidder.id, status, note);
    setActionMessage(`Officer decision '${status}' recorded in official audit register.`);
    setNote('');
  };

  const statusBadge = (status: CheckStatus) => {
    if (status === 'Verified') {
      return <span className="inline-block px-2 py-0.5 bg-[#F0FDF4] text-[#15803D] border border-[#BBF7D0] rounded-[2px] text-[11px] font-semibold">[✓] Verified</span>;
    }
    if (status === 'Failed') {
      return <span className="inline-block px-2 py-0.5 bg-[#FEF2F2] text-[#B72025] border border-[#FCA5A5] rounded-[2px] text-[11px] font-semibold">[✕] Failed</span>;
    }
    if (status === 'Needs Review') {
      return <span className="inline-block px-2 py-0.5 bg-[#EFF6FF] text-[#1D4ED8] border border-[#BFDBFE] rounded-[2px] text-[11px] font-semibold">[!] Review required</span>;
    }
    if (status === 'Not Applicable') {
      return <span className="inline-block px-2 py-0.5 bg-[#F8F9FA] text-[#475569] border border-[#D9DDE3] rounded-[2px] text-[11px] font-semibold">Not applicable</span>;
    }
    return <span className="inline-block px-2 py-0.5 bg-[#FFFBEB] text-[#D97706] border border-[#FDE68A] rounded-[2px] text-[11px] font-semibold">[○] Pending</span>;
  };

  const verifiedCount = bidder.requirements.filter((r) => r.status === 'Verified').length;
  const exceptionCount = bidder.requirements.filter((r) => r.status === 'Failed' || r.status === 'Needs Review').length;
    ...
    <div className="space-y-4 font-sans pb-8">
      ...
      <div className="flex items-center justify-between text-xs text-[#475569] border-b border-[#D9DDE3] pb-2.5">
        <div className="flex items-center gap-1.5">
          <Link to="/tenders" className="hover:underline text-[#0B2A4A] flex items-center gap-1 font-semibold">
        ...
        <button
          ...
          className="px-3 py-1 bg-[#0B2A4A] hover:bg-[#123B63] text-white text-xs font-semibold rounded-[2px] flex items-center gap-1.5 transition-colors cursor-pointer"
        >
          {verifying ? (
      ...
      {actionMessage && (
        <div className="p-3 border border-[#BBF7D0] bg-[#F0FDF4] text-[#15803D] text-xs font-medium rounded-[2px] flex items-center justify-between">
          ...
          <button onClick={() => setActionMessage(null)} className="text-[#15803D] text-xs underline cursor-pointer">
            Dismiss
          </button>
        </div>
      )}
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[#475569]">
                • Bidder verification workspace
              </span>
            </div>
            <h1 className="font-serif font-bold text-xl text-[#0B2A4A] mt-1">
              {bidder.name}
            </h1>
          ...
          <div className="flex items-center border border-[#D9DDE3] divide-x divide-[#D9DDE3] bg-[#F8F9FA] rounded-[2px] text-xs">
            <div className="px-3.5 py-2 text-center">
              <span className="block text-[10px] uppercase font-semibold text-[#475569]">Status</span>
              <strong className="text-xs text-[#0B2A4A] block mt-0.5">{bidder.status}</strong>
            </div>
            <div className="px-3.5 py-2 text-center bg-white">
              <span className="block text-[10px] uppercase font-semibold text-[#475569]">Compliance score</span>
              <strong className="text-base font-semibold text-[#0B2A4A] font-mono block mt-0.5">
                {bidder.score || '0'}/100
              </strong>
            </div>
            <div className="px-3.5 py-2 text-center">
              <span className="block text-[10px] uppercase font-semibold text-[#475569]">Risk assessment</span>
              <span
              ...
              >
                {bidder.risk === 'HIGH' ? '[!] High' : bidder.risk === 'MEDIUM' ? '[!] Medium' : '[✓] Low'}
              </span>
            </div>
          </div>
        </div>

        {/* Evidence-Based Subtext Strip */}
        <div className="mt-3 pt-2.5 border-t border-[#E6E9EF] text-xs text-[#475569] flex flex-wrap items-center gap-3">
          <span>
            <strong>Assessment basis:</strong> {bidder.requirements.length} statutory requirements evaluated
          </span>
          <span>•</span>
      ...
      <div className="grid lg:grid-cols-3 gap-4 items-start">
        {/* Left 2 Cols: Requirements Verification Matrix Table */}
            ...
            <div className="px-4 py-3 border-b border-[#D9DDE3] flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px] text-[#475569]">
                  Evaluated criteria, extracted evidence from submitted documents, and officer review state.
                </p>
              </div>
            </div>
                    ...
                    <React.Fragment key={req.id}>
                      <tr className="hover:bg-[#F0F4F8] transition-colors">
                        <td>
                          ...
                          <span className="text-[11px] text-[#475569] block mt-0.5">{req.note}</span>
                        </td>
                        <td>
                          ...
                          <td colSpan={5} className="p-3 border-b border-[#D9DDE3]">
                            <div className="bg-white p-3 border border-[#CBD2DE] rounded-[2px] text-xs space-y-2">
                              <div className="flex items-center justify-between border-b border-[#E6E9EF] pb-1.5">
                                <span className="font-bold text-[#0B2A4A]">Evidence audit trail:</span>
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1 text-[11px]">
                                <div>
                                  <span className="text-[#475569] block text-[10px]">Source document:</span>
                                  <strong className="text-[#202124]">{req.evidence}</strong>
                                </div>
                                <div>
                                  <span className="text-[#475569] block text-[10px]">Extraction method:</span>
                                  <strong className="text-[#202124]">PyMuPDF + Regex Parser</strong>
                                </div>
                                <div>
                                  <span className="text-[#475569] block text-[10px]">Confidence level:</span>
                                  <strong className="text-[#15803D]">96% (High)</strong>
                                </div>
                              </div>
                              <div className="pt-2 flex items-center justify-between border-t border-[#E6E9EF]">
                                <span className="text-[11px] text-[#475569]">Manual officer override:</span>
                                <select
                                  value={req.status}
                                  onChange={(e) => updateRequirement(bidder.id, req.id, e.target.value as CheckStatus)}
                ...
                {bidder.discrepancies.map((d, idx) => (
                  <div key={idx} className="text-xs space-y-1.5 pt-2 first:pt-0">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-1.5 py-0.2 bg-[#FEE2E2] text-[#991B1B] border border-[#FCA5A5] rounded-[2px] font-mono text-[10px] font-bold">
                        SEVERITY: {d.severity}
                      </span>
                    </div>
                    ...
                    {d.expected && d.found && (
                      <div className="grid grid-cols-2 gap-2 bg-[#FFF8F8] border border-[#FECDD3] p-2 rounded-[2px] text-[11px]">
                        <div>
                          <span className="block font-semibold text-[#475569] text-[10px]">What was expected:</span>
                          <span className="font-mono text-[#0B2A4A]">{d.expected}</span>
                        </div>
                        <div>
                          <span className="block font-semibold text-[#475569] text-[10px]">What was found:</span>
        ...
        {/* Right Column: Recommendations & Officer Decision Box */}
        <aside className="space-y-4">
          ...
          <section className="bg-white border border-[#D9DDE3] p-4 rounded-[2px]">
            ...
            <ul className="mt-3 space-y-2 text-xs text-[#334155]">
              {(bidder.recommendations || [
                ...
                <li key={idx} className="bg-[#F8F9FA] p-2 border border-[#E6E9EF] rounded-[2px] flex items-start gap-1.5 text-[11px] leading-relaxed">
                  <span className="text-[#0B2A4A] font-bold font-mono">#{idx + 1}</span>
                  <span>{rec}</span>
                </li>
              ))}
            </ul>
          ...
          {/* Official Procurement Officer Decision Box */}
          <section className="bg-white border border-[#D9DDE3] p-4 rounded-[2px]">
            ...
            <textarea
              ...
              className="w-full border border-[#CBD2DE] p-2 text-xs rounded-[2px] min-h-20 focus:outline-[#0B2A4A]"
              placeholder="Record the official basis and reasoning for the qualification or disqualification decision..."
            />

            <div className="mt-3 grid grid-cols-2 gap-1.5">
              <button
                onClick={() => handleDecision('Qualified')}
                className="px-2.5 py-1.5 bg-[#15803D] hover:bg-[#166534] text-white text-xs font-semibold rounded-[2px] transition-colors cursor-pointer text-center"
              >
                Confirm qualified
              </button>
              <button
                onClick={() => handleDecision('Disqualified')}
                className="px-2.5 py-1.5 bg-[#B72025] hover:bg-[#991B1B] text-white text-xs font-semibold rounded-[2px] transition-colors cursor-pointer text-center"
              >
                Record disqualification
              </button>
              <button
                onClick={() => handleDecision('Clarification Requested')}
                className="px-2.5 py-1.5 bg-white text-[#92400E] border border-[#FDE68A] hover:bg-[#FFFBEB] text-xs font-semibold rounded-[2px] transition-colors cursor-pointer text-center"
              >
                Request clarification
              </button>
              <button
                onClick={() => handleDecision('Under Review')}
                className="px-2.5 py-1.5 bg-white text-[#0B2A4A] border border-[#CBD2DE] hover:bg-[#F0F4F8] text-xs font-semibold rounded-[2px] transition-colors cursor-pointer text-center"
              >
                Mark for review
              </button>
            </div>
