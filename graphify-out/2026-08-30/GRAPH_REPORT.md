...
## Corpus Check
- 170 files · ~128,063 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1658 nodes · 3323 edges · 107 communities (95 shown, 12 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 53 edges (avg confidence: 0.93)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0cba2be6`
...
Cohesion: 0.07
Nodes (34): get_risk_level_from_probability(), Convert a delay probability (0.0–1.0) to a risk level label., build_feature_vector(), Extract feature values from a project data dictionary. Args: project_data:…, get_feature_importance(), Return feature importance dict, or empty dict if not available., _deterministic_fallback(), _estimate_delay_days() (+26 more)
...
Cohesion: 0.10
Nodes (28): get_dashboard_metrics(), get, Returns aggregate operational statistics computed from the live database. Every…, _create_alert(), Insert a new alert if one of the same type for this case doesn't already exist.…, Evaluate a single case and generate/update alerts as appropriate. Called…, refresh_alerts_for_case(), _create_audit_log() (+20 more)
...
Cohesion: 0.11
Nodes (35): AlertOut, AuditLogOut, BottleneckAnalysis, CaseEventOut, CaseStage, CaseStatus, DashboardMetrics, DepartmentInfo (+27 more)
...
Cohesion: 0.07
Nodes (39): post, UploadFile, Trigger OCR processing on an already-uploaded document. Pipeline: 1. Retrieve…, Upload a document and attach it to a case. - Validates file type (PDF, PNG,…, run_ocr_on_document(), upload_document(), create_project(), _enrich_project_with_ml() (+31 more)
...
Cohesion: 0.18
Nodes (13): App(), VerificationLanding(), useProcurement(), ProcurementAuditTrail(), ProcurementDashboard(), ExtractedField, ProcurementDocuments(), Search() (+5 more)

### Community 42 - "list_alerts"
Cohesion: 0.67
...
### Community 99 - "apiClient.ts"
Cohesion: 0.32
Nodes (6): AppSidebar(), NAV_ITEMS, Settings(), isUsingMockApi(), mockApi, realApi

### Community 100 - "UploadDocument.tsx"
Cohesion: 0.25
Nodes (6): OCR_STEPS, RegPhase, SuccessResult, ocrService, Department, DocumentType

### Community 101 - "TestPriorityAndDashboardAggregation"
Cohesion: 0.33
...
## Knowledge Gaps
- **376 isolated node(s):** `$schema`, `typescript`, `oxc`, `react/rules-of-hooks`, `warn` (+371 more)
...
- **Why does `get_supabase()` connect `get_supabase` to `predict_delay`, `main.py`, `mark_alert_read`, `list_alerts`, `case_service.py`, `get_document`, `auth.py`, `cases.py`, `projects.py`, `test_land_acquisition.py`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `validate_file()` connect `validate_file` to `test_documents.py`, `procurement.py`, `.test_validate_file_returns_none_on_success`, `.test_validate_file_returns_string_on_error`, `process_ocr`, `projects.py`, `deadline_service.py`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `extract_fields_from_text()` connect `TestFieldExtraction` to `test_documents.py`, `ocr_service.py`, `extract_procurement_fields`, `projects.py`, `deadline_service.py`?**
